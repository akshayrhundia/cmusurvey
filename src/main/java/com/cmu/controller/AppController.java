...
public class AppController {
    ...
    public String getWritesurvey(@PathVariable("qId") String qId, @RequestParam("user") String username, ModelMap model,
        ...
        if (type.equalsIgnoreCase("text")) {
            try {
                List<Integer> qidTextList = questionService.findAllTextQids();
                if (Integer.parseInt(qId) < qidTextList.size()) {
                    QuestionText que = questionService.findTextById(qidTextList.get(Integer.parseInt(qId)));
                    model.addAttribute("question", que);
                    model.addAttribute("qId", Integer.parseInt(qId) + 1);
                    return "wsurvey";
                } else {
            ...
            try {
                List<String> qidAudioTextList = questionService.findAllAudioForTextQids();
                if (Integer.parseInt(qId) < qidAudioTextList.size()) {
                    QuestionAudioForText que = questionService.findAudioForTextById(qidAudioTextList.get(Integer.parseInt(qId)));
                    model.addAttribute("question", que);
                    model.addAttribute("qId", Integer.parseInt(qId) + 1);
        ...
        /*else {
            try {
                List<String> qidAudioAudioList = questionService.findAllAudioForAudioQid();
                if (Integer.parseInt(qId) < qidAudioAudioList.size()) {
                    QuestionAudioForAudio que = questionService.findAudioForAudioById(qidAudioAudioList.get(Integer.parseInt(qId)));
                    model.addAttribute("question", que);
                    model.addAttribute("qId", Integer.parseInt(qId) + 1);
                    return "wsurvey";
                } else {
        ...
        if (qType.equalsIgnoreCase("text")) {
            ...
            try {
                ...
                if (que != null) {
                    ...
                    return new RedirectView("writesurvey/" + (qIndex) + "?user=" + username);
                } else {
                    return new RedirectView("error");
                }
            } catch (Exception ex) {
    ...
    public String saveAnswerAudio(HttpServletRequest request, @RequestParam("reply") MultipartFile reply,
        ...
        if (qType.equalsIgnoreCase("text")) {
            ...
            if (que != null) {
                ...
                return "../speaksurvey/" + qIndex + "?user=" + username;
            } else {
                return "error";
            }
        }
    ...
    @RequestMapping(value = {"/qidtextquestions"}, method = RequestMethod.GET)
    @ResponseBody
    public String qidtextquestions() {
        List<Integer> myList = questionService.findAllTextQids();
        return new Gson().toJson(myList);
    }

    @RequestMapping(value = {"/qidlistaudiofortextquestions"}, method = RequestMethod.GET)
    @ResponseBody
    public String qidlistaudiofortextquestions() {
        List<String> myList = questionService.findAllAudioForTextQids();
        return new Gson().toJson(myList);
    }

    @RequestMapping(value = {"/qidlistaudioforaudioquestions"}, method = RequestMethod.GET)
    @ResponseBody
    public String qidlistaudioforaudioquestions() {
        List<String> myList = questionService.findAllAudioForAudioQid();
        return new Gson().toJson(myList);
    }
    ...
    @RequestMapping(value = {"/deleteallquestionsbytype/{qtype}/{type}"}, method = RequestMethod.POST)
    @ResponseBody
    public String deleteallquestionsbytype(@PathVariable("qtype") String qtype, @PathVariable("type") String type) {
        if(type.equalsIgnoreCase("audio")){
            type = "Audio";
        }
        userAnsService.deleteAllUserAnswersByQuestionType(qtype, type);
        return "success";
    }

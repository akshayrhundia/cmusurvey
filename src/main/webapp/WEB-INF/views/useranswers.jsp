				...
				<tbody id="main-body">
					<c:forEach items="${answers}" var="res" varStatus="counter">
						<tr>
							...
							<c:choose>
								<c:when test = "${type == 'audiotext'}">
									<td><audio controls><source src="../getQuestionAudioForTextFile/AUDIOTEXT_0${res.ans.qId}" type="audio/wav" /></td>
      							</c:when>
      							<c:when test = "${type == 'audioaudio'}">
									<td><audio controls><source src="../getQuestionAudioForAudioFile/AUDIOAUDIO_0${res.ans.qId}" type="audio/wav" /></td>
      							</c:when>
      							<c:otherwise>
      								<td>${res.ans.qtype}</td>
    							</c:otherwise>
